...
regex_pattern = re.compile(rf'facs_{facs_start}_\S*')
...
# Manually close the <table> tag and include </row> before </table> before the closing </div> tag
# Use regex to insert the closing </row> and </table> tags
tei_str = re.sub(r'(<table[^>]*>.*?)(<row[^>]*>.*?)(</div>)', r'\1\2</row></table>\3', tei_str, flags=re.DOTALL)
